...
**PackageReference**
```xml
<PackageReference Include="Syrx.Commanders.Databases.Connectors.MySql.Extensions" Version="2.4.5" />
```
...
services.UseSyrx(builder => builder
    .UseMySql(mysql => mysql
        ...
        .AddCommand(types => types
            .ForType<UserRepository>(methods => methods
                .ForMethod("GetUsers", command => command
                    .UseConnectionAlias("Slave"))    // Read operations
                .ForMethod("CreateUser", command => command
                    .UseConnectionAlias("Master"))))); // Write operations
```
